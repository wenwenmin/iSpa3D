...
def read_10X_Visium(path,
    ...
    # adata.obs['original_clusters'] = adata.obs['original_clusters'].astype(int).astype(str)
...
def readmerfish(root_dir=None, section_id=None,
                library_id="MERSEQ",
                scale=None,
                quality="hires",
                spot_diameter_fullres=50,
                background_color="white"):
    """
    读取MERFISH数据，使用已对齐的坐标系

    Args:
        root_dir: 数据根目录
        section_id: 切片ID，如'0.04','0.09'等
        library_id: 库ID
        scale: 缩放比例
        quality: 图像质量
        spot_diameter_fullres: 点直径
        background_color: 背景颜色
    """
    # 读取h5ad数据
    adata = sc.read_h5ad(os.path.join(r"C:\Code\Data\MERFISH\hypothalamus" + section_id+ ".h5ad"))
    adata.var_names_make_unique()
    adata.obs_names = adata.obs_names.astype(str).str.strip()

    # 读取PASTE对齐后的坐标
    coord_path = r"C:\Code\Data\MERFISH/paste_new_coord_0.04_0.09_0.14_0.19_0.24.csv"
    aligned_coords = pd.read_csv(coord_path, index_col=0)
    aligned_coords.index = aligned_coords.index.astype(str).str.strip()
    aligned_coords['slice'] = aligned_coords['slice'].astype(str)

    # 过滤出当前切片的坐标
    slice_coords = aligned_coords[aligned_coords['slice'] == section_id]

    # 检查坐标是否与当前adata匹配
    common_cells = np.intersect1d(adata.obs_names, slice_coords.index)
    print(f"✅ 当前切片与坐标共通细胞数: {len(common_cells)} / {adata.shape[0]}")

    # 更新坐标
    new_spatial = np.zeros((adata.shape[0], 2))
    for i, cell_id in enumerate(adata.obs_names):
        if cell_id in slice_coords.index:
            new_spatial[i, 0] = slice_coords.loc[cell_id, 'x']
            new_spatial[i, 1] = slice_coords.loc[cell_id, 'y']
    ...
    adata.obs["imagecol"] = adata.obsm["spatial"][:, 0] * scale
    adata.obs["imagerow"] = adata.obsm["spatial"][:, 1] * scale
    if adata.obs["imagecol"].isna().any() or adata.obs["imagerow"].isna().any():
        # 移除包含NaN的行
        adata = adata[~adata.obs["imagecol"].isna() & ~adata.obs["imagerow"].isna()].copy()
    # 创建图像
    max_size = np.max([adata.obs["imagecol"].max(), adata.obs["imagerow"].max()])
    max_size = int(max_size + 0.1 * max_size)
    if background_color == "black":
