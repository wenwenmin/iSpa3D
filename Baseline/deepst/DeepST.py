...
class run():
    ...
    def _get_adata(
        ...
        assert platform in ['Visium', 'ST', 'MERFISH', 'slideSeq', 'stereoSeq', 'benchmark_test']
        ...
        elif platform == 'benchmark_test':
            if data_name in ['-0.04', '-0.09', '-0.14', '-0.19', '-0.24', '-0.29']:
                adata = readmerfish(root_dir=data_path, section_id=data_name)
            else:
    ...
    def _get_cluster_data(
        ...
        # 构建邻接图（有些后续方法可能依赖 neighbors）
        sc.pp.neighbors(adata, use_rep='DeepST_embed')

        # 使用 STAGATE embedding 作为 Mclust 输入
        used_obsm = 'DeepST_embed'

        # 使用 mclust 进行聚类
        adata = mclust_R(
            adata=adata,
            num_cluster=n_domains,
            modelNames='EEE',  # 或 'VVV', 'EEE', 'EEE' 是最常用高斯模型之一
            used_obsm=used_obsm,
            random_seed=0
        )

        # 将 mclust 结果映射到统一的键名上，供 refine 使用
        adata.obs["DeepST_domain"] = adata.obs["mclust"].astype(str)
