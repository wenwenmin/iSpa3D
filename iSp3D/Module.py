...
class ContrastiveHead(nn.Module):
    """
    Cluster-level probability projection: latent_dim -> num_cluster
    """
    def __init__(self, in_dim, num_cluster, hidden_dim=None, temperature=0.5):
        super().__init__()
        ...
        self.net = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(hidden_dim, num_cluster, bias=True),
        )
        self.temperature = temperature

    def forward(self, z):
        logits = self.net(z)                 # [N, K]
        probs = F.softmax(logits / self.temperature, dim=1)  # [N, K]
        return logits, probs
...
class ClusterLoss(nn.Module):
    ...
    def __init__(self, num_cluster, temperature):
        super(ClusterLoss, self).__init__()
        self.num_cluster = num_cluster
        self.temperature = temperature

        # Mask for correlated cluster pairs
        mask = self.mask_correlated_clusters(num_cluster)
        self.register_buffer('mask', mask)
        self.similarity_f = nn.CosineSimilarity(dim=2)

    def mask_correlated_clusters(self, num_cluster):
        """
        Create mask matrix to filter out positive and self-contrast pairs
        """
        N = 2 * num_cluster
        mask = torch.ones((N, N), device="cpu")
        mask.fill_diagonal_(0)

        # Mask cross-view same-cluster pairs
        for i in range(num_cluster):
            mask[i, num_cluster + i] = 0
            mask[num_cluster + i, i] = 0
        return mask.bool()

    def forward(self, c_i, c_j):
        device = c_i.device

        # 1) Global balance: KL(p_mean || uniform)
        probs_i = c_i.clamp_min(1e-8)
        probs_j = c_j.clamp_min(1e-8)

        p_i = probs_i.mean(0)
        p_j = probs_j.mean(0)

        u = torch.full_like(p_i, 1.0 / self.num_cluster)
        ne_i = F.kl_div(p_i.log(), u, reduction='batchmean')
        ne_j = F.kl_div(p_j.log(), u, reduction='batchmean')
        ne_loss = ne_i + ne_j

        # 2) Cluster-wise contrastive loss
        ci = F.normalize(probs_i.t(), dim=1)
        cj = F.normalize(probs_j.t(), dim=1)
        c = torch.cat((ci, cj), dim=0)        # [2K, B]
...
class Autoencoder(nn.Module):
    def __init__(self, input_dim, config, num_cluster=None):
        super().__init__()
        ...
        if num_cluster is not None:
            self.cl_head = ContrastiveHead(
                in_dim=config['latent_dim'],
                num_cluster=num_cluster,
                hidden_dim=config.get('cluster_head_hidden', None),
                temperature=config.get('cl_temperature', 0.5)
            )

            self.cluster_loss_fn = ClusterLoss(
                num_cluster=num_cluster,
                temperature=config.get('cluster_temperature', 0.5)
            )

        self.w_js = config.get('w_js', 0.0)  
        self.w_nce = config.get('w_nce', 1.0)  
        self.diff_only_neg = config.get('diff_cluster_neg_only', True)
    ...
    def _complement_masks_(self, x, edge_index, mask_rate=0.3):
        num_nodes = x.shape[0]
        perm = torch.randperm(num_nodes, device=x.device)
        ...
        def _apply(view_nodes):
            out = x.clone()
            if self.replace_rate > 0:
                n = view_nodes.numel()
                num_noise = int(self.replace_rate * n)
                pm = torch.randperm(n, device=x.device)
                token_nodes = view_nodes[pm[: int(self.mask_token_rate * n)]]
                noise_nodes = view_nodes[pm[-num_noise:]]
                noise_from = torch.randperm(num_nodes, device=x.device)[:num_noise]
                out[token_nodes] = 0.0
                out[noise_nodes] = x[noise_from]
            else:
                out[view_nodes] = 0.0
            out[view_nodes] += self.enc_mask_token
            return out

        x_main = _apply(mask_nodes)
        x_comp = _apply(keep_nodes)
        return x_main, x_comp, edge_index.clone(), mask_nodes, keep_nodes

    def _pair_info_nce_(self, p1, p2, tau=0.5, diff_only_neg=True):
        p1 = F.normalize(p1, dim=1)
        p2 = F.normalize(p2, dim=1)
        logits = (p1 @ p2.t()) / tau
        N = logits.size(0)
        pos = torch.arange(N, device=logits.device)

        if diff_only_neg:
            y = p2.argmax(dim=1)  # [N]
            same = y.unsqueeze(0).eq(y.unsqueeze(1))  # [N, N]
            eye = torch.eye(N, device=logits.device, dtype=torch.bool)
            mask = (~same) | eye  # 允许正样本的 i==i，其它同簇屏蔽
            logits = logits.masked_fill(~mask, -1e9)

        loss12 = F.cross_entropy(logits, pos)
        loss21 = F.cross_entropy(logits.t(), pos)
        return 0.5 * (loss12 + loss21)

    def contrastive_forward(self, x, edge_index):
        if self.num_cluster is None:
            return torch.tensor(0.0, device=x.device), None, None
        
        x_main, x_comp, use_adj, _, _ = self._complement_masks_(x, edge_index, self.mask_rate)
        z_main = self.encoder(x_main, use_adj)  # [N, d]
        z_comp = self.encoder(x_comp, use_adj)  # [N, d]
        logit_main, p_main = self.cl_head(z_main)
        logit_comp, p_comp = self.cl_head(z_comp)

        cluster_loss = self.cluster_loss_fn(p_main, p_comp)
        
        return cluster_loss, z_main, z_comp
...
class Discriminator(nn.Module):
    def __init__(self, class_num, label, config):
        ...
        self.discriminator.add_module('discriminator_L1', full_block(self.latent_dim, self.dic_hidden, self.p_drop,
                                                                     act=create_activation("lrelu")))
        self.discriminator.add_module('discriminator_L2', full_block(self.dic_hidden, self.dic_hidden, self.p_drop,
                                                                     act=create_activation("lrelu")))
